...
export class Canvas {
  ...
  /** Row height in px, kept equal to the column width so grid cells are square. */
  readonly cellSize = signal(80);
  ...
  constructor() {
    ...
    afterNextRender(() => {
      ...
      const updateCellSize = () =>
        this.cellSize.set(canvasEl.getBoundingClientRect().width / COLS);
      updateCellSize();
  ...
  itemStyle(comp: ComponentDef): Record<string, string> {
    ...
    return {
      ...
      top: `${comp.row * cell}px`,
      height: `${comp.h * cell}px`,
    };
  }
  ...
  onDrop(event: CdkDragDrop<ComponentDef[]>): void {
    ...
    const relX = Math.max(
      0,
      Math.min(event.dropPoint.x - rect.left, rect.width - 1),
    );
    const relY = Math.max(0, event.dropPoint.y - rect.top);

    const col = asCol(
      Math.min(COLS - 1, Math.floor((relX / rect.width) * COLS)),
    );
    const row = asRow(Math.floor(relY / this.cellSize()));
  ...
  onItemMouseDown(event: MouseEvent, comp: ComponentDef): void {
    if (
      (event.target as HTMLElement).closest(
        '.canvas__remove, .canvas__resize-handle',
      )
    )
      return;
    ...
    const onUp = (e: MouseEvent) => {
      ...
      const canvasWidth =
        this.canvasRef()!.nativeElement.getBoundingClientRect().width;
      const dx = e.clientX - startX;
      const dy = e.clientY - startY;
      ...
      const newCol = Math.max(
        0,
        Math.min(maxCol, Math.round(comp.col + (dx / canvasWidth) * COLS)),
      );
      const newRow = Math.max(0, Math.round(comp.row + dy / this.cellSize()));
  ...
  onResizeStart(event: MouseEvent, comp: ComponentDef): void {
    ...
    const onUp = (e: MouseEvent) => {
      ...
      const newW = Math.max(
        ...
        Math.min(
          COLS,
          Math.round(comp.w + ((e.clientX - startX) / canvasWidth) * COLS),
        ),
      );
      const newH = Math.max(
        1,
        Math.round(comp.h + (e.clientY - startY) / this.cellSize()),
      );
