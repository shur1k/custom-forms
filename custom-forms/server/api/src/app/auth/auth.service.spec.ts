...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('returns an accessToken when registration succeeds', async () => {
      ...
      db.query.roles.findFirst.mockResolvedValue({ id: 'role-uuid', name: 'user' });
      db.insert.mockReturnValue({
        values: jest.fn().mockReturnValue({
          returning: jest.fn().mockResolvedValue([{ id: 'user-uuid', email: 'a@b.com' }]),
        }),
      });

      const result = await service.register({ email: 'a@b.com', password: 'Pass1234!' });

      expect(result).toEqual({ accessToken: 'mock.jwt.token' });
      expect(jwtService.sign).toHaveBeenCalledWith({ sub: 'user-uuid', email: 'a@b.com' });
    });

    it('throws ConflictException when email already exists', async () => {
      db.query.users.findFirst.mockResolvedValue({ id: 'existing', email: 'a@b.com' });

      await expect(service.register({ email: 'a@b.com', password: 'Pass1234!' }))
  ...
  describe('login', () => {
    it('returns an accessToken when credentials are valid', async () => {
      ...
      db.query.users.findFirst.mockResolvedValue({ id: 'user-uuid', email: 'a@b.com', passwordHash: hash });
